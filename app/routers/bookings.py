...
# concurrent requests can't both pass checks that read stale state, and
# serializes the cancel critical section so a booking can't be cancelled
# (and refunded) twice.
...
def create_booking(
    ...
    # Run the slow warmup/audit steps before entering the critical section so
    # the lock is held only for the checks and insert themselves.
    _pricing_warmup()
    _quota_audit()
